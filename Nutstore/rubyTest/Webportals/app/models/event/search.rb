...
class Event < ActiveRecord::Base
  class Search < Struct.new(:query, :tag, :order, :current)
    ...
    # grouped_by是根据block里面的规则分类,分类后形成一个hash,分类的结果是键名key,分类的元素为值value
    # :current? 用符号表示一个方法current?, &符号把这个方法(对象)用to_proc转换为proc对象,相当一个block,为了能被grouped_by使用
    def grouped_events
    ...
    # 调用父类的current,判断返回的结果是否有1或者ture
    def current
