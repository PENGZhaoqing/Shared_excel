...
module MappingHelper
  def map_provider
    Webportal.mapping_provider || 'stamen'
  end
  ...
  def map_provider_dependencies
    {
      ...
      "google" => [
        "https://maps.googleapis.com/maps/api/js?key=#{Webportal.mapping_google_maps_api_key}&sensor=false",
  ...
  class Map < Struct.new(:items, :context, :options)
    ...
    def marker_color
      Webportal.mapping_marker_color
    end
    ...
    def map_tiles
      Webportal.mapping_tiles || 'terrain'
    end
  end
end
