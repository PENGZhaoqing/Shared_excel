  ...
  module EventsHelper
    ...
    #  params
    #  => {"utf8"=>"✓", "query"=>"da", "controller"=>"calagator/events", "action"=>"search"}

    #  params.merge(:order => key)
    #  =>  {"utf8"=>"✓", "query"=>"da", "controller"=>"calagator/events", "action"=>"search", "order"=>"name"}

    #  params.tap { |o| o.delete :order }
    # => {"utf8"=>"✓", "query"=>"da", "controller"=>"calagator/events", "action"=>"search"}
    ...
    def google_events_subscription_link(filter={})
      ...
      # link=> "http://127.0.0.1:3000/events/search.ics?query=da"
      # CGI.escape(link)=> "http%3A%2F%2F127.0.0.1%3A3000%2Fevents%2Fsearch.ics%3Fquery%3Dda"

      #  "#{GOOGLE_EVENT_SUBSCRIBE_BASE}#{CGI.escape(link)}"
      # => "http://www.google.com/calendar/render?cid=http%3A%2F%2F127.0.0.1%3A3000%2Fevents%2Fsearch.ics%3Fquery%3Dda"

      "#{GOOGLE_EVENT_SUBSCRIBE_BASE}#{CGI.escape(link)}"

      # "http://www.baidu.com"
      # => No route matches [GET] "/events/www.baidu.com"

    end

    # Returns an iCalendar subscription URL.
    def icalendar_feed_link(filter={})
      # filter=> {:query=>"da", :tag=>nil}
