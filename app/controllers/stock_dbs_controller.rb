class StockDbsController < ApplicationController
  ...
  def index
    if logged_in_visit?
      @stock_dbs=StockDb.filter_by_type(current_visit.supplier2).search(search_params).order('created_at desc').paginate(:page => params[:page], :per_page => 20)
    elsif logged_in?
      @stock_dbs=StockDb.search(search_params).order('created_at desc').paginate(:page => params[:page], :per_page => 20)
    else
